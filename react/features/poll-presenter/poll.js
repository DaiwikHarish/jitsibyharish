...
if(apibyseconds>=3)
{
    ...
    fetch(
        ApiConstants.poll+"?groupId="+pollSeletedId+"&includeStatistic=true")
 ...
 let interval=  setInterval(() => {
      ...
      const getDeadTime = () => {
          ...
          polllauched?
          deadline.setSeconds(deadline.getSeconds() + parseInt(pollcounttime)):null;
      ...
      function getPoll() {
        ...
        setisDisabledSelect(false)
        fetch(
            ApiConstants.pollGroupbyMeeting
        )
            .then((response) => response.json())
            .then((pollData) => {
...
<div className="poll-header"><div className="poll-question"><span>{item.question}</span></div>



</div><ol className="poll-answer-list">
{
item.answers.map((ans, index) => (
<li className="poll-answer-container"><div className='pollFormControl'>

<label className='pollActiveArea'>
 <input  className={ item.changingVote ?"pollRadio":"pollcheckbox"} type="checkbox" />
 ...
 </div></label><label>{ans.name}</label></div></li>
))
}
</ol>
...
<div><div className="poll-results">
 <div className="poll-header"><div className="poll-question">
     <strong>{item.question}</strong></div>
     
     
     </div>
     <div style={{paddingBottom:10, color:'#246fe5'}}>{item.totalUsersAnswered} ({parseInt(item.usersAnsweredPercentage)}%) answered</div>
     ...
     item.answers.map((ans, index) => (
                     ...
                     <li>
                         <div className="poll-answer-header">
                             <span className="poll-answer-vote-name">{ans.name}</span></div>
                             <div className="poll-answer-short-results">
                                 <span className="poll-bar-container">
                                     <div className="poll-bar" style={{width: ans.pollPercentage+'%'}}></div>
                                     </span><div className="poll-answer-vote-count-container">
                                         <span className="poll-answer-vote-count"> {ans.pollStatistics}  {parseInt(ans.pollPercentage)+'%'}</span>
                                         </div>
                                         </div>
                                         </li>
     ))                                    
          ...
          const relaunchPoll = () => {
            setLoading(true)
              const DELETEMethod = {
                 ...
                 fetch(ApiConstants.poll+"?groupId="+pollSeletedId+"&pollTableOnly=true", DELETEMethod)
                 .then(response => response.json())
                 .then(data => {
                    launchPoll()
                 })
...
// updatedUserId: "4"
    ...
    return (
        ...
        getOptionLabel={(option) => `${option.label} - (total questions:  ${option.questionCount})`}
        onChange={onChange}
...
{/* <h2>{timer}</h2> */}
<h2 style={{fontSize:26, display:'flex', fontWeight:'500', color:'red'}}>
...
<Icon
    ...
    {pollcounttime!=0?timer:timerCount}
                    ...
                    <button aria-label="EndPoll"  onClick={endedPoll}  className="pollbtn" title="EndPoll" type="button">End Poll</button>
...
<Icon
    ...
    <Input
          type='number' 
          value = { pollcounttime }
                onChange={val => setPollcounttime(val)} 
                placeholder = "Poll Duration in Seconds"/></div>
{!pollRelaunch?
