...
function _handleReceivedMessage({ dispatch, getState }: IStore,
    ...
    // skip message notifications on join (the messagesQa having timestamp - coming from the history)
...
function _shouldSendPrivateMessageTo(state: IReduxState, action: AnyAction) {
    ...
    const { messagesQa, privateMessageRecipient } = state['features/chat'];
    ...
    // But messagesQa may come rapidly, we want to protect our users from mis-sending a message
