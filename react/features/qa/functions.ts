...
/**
 * Selector for calculating the number of unread chat messagesQa.
 *
...
export function getUnreadCount(state: IReduxState) {
    ...
    if (navigator.product === 'ReactNative') {
        // React native stores the messagesQa in a reversed order.
        lastReadIndex = messagesQa.indexOf(<IMessage>lastReadMessage);

        for (let i = 0; i < lastReadIndex; i++) {
            if (messagesQa[i].isReaction) {
                reactionMessages++;
            }
        }
    ...
    lastReadIndex = messagesQa.lastIndexOf(<IMessage>lastReadMessage);
