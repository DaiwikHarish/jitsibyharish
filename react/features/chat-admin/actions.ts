...
import { _loadAttendees, _selectedAttendee, _sendChatMessage, _updateAttendee } from './functions';
 ...
 */
export function selectedAttendee(attendeeId:string) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        // Step 1: do process 
...
export function sendChatMessage(newMsg:string) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        // Step 1: do process 
